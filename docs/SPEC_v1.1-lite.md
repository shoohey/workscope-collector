...
- 方式：サービスアカウント（OAuth不要、顧客にGoogleログインさせない）
- キー：顧客ごとにサービスアカウント発行 → JSONキーをEXEに暗号化埋め込み
...
[Google Drive 共有ドライブに /WorkScope/tribe-001/ 作成]
[サービスアカウント発行 → 共有ドライブに編集権限付与]
...
| トレイ | pystray, Pillow |
| GDriveクライアント | google-api-python-client, google-auth |
| 暗号化 | cryptography（SAキー埋め込み用） |
| ログ | structlog |
...
├── scripts/
│   ├── build_for_customer.sh  ← SAキー埋め込み手順を追加
│   └── ...
├── docs/
...
| 認証情報 | サービスアカウントJSONはEXEにcryptography暗号化埋め込み |
...
:: 2. テスト用Google共有ドライブ＋サービスアカウント準備
::    - test-tribe-001 の共有ドライブ作成
::    - サービスアカウント発行 → JSONキーをローカル保存
```
